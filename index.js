...
function capitalizeNames() {
  ...
  const capitalizeNamesArray = []
  items.map((item) => {
    let firstLetter = item.name[0].toUpperCase()
    let restOfTheWord = item.name.slice(1,item.name.length)
    capitalizeNamesArray.push(firstLetter + restOfTheWord)
  })
  return capitalizeNamesArray
}
...
function calculateTotalInventory() {
  ...
  const sumWithInitial = items.reduce(
    (accumulator, item) => (accumulator + item.inventory), initialValue
    );
...
function calculateTotalCartPrice() {
  ...
  for (let i= 0; i < cart.length; i++) {
    const totalPriceInCart = cart.reduce((accumulator,item) => accumulator + (item.price * item.quantity), 0)
